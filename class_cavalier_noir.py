...
class cavalier_noir(pygame.sprite.Sprite):
    ...
    def __init__(self,game,x,y,num):
        ...
        self.image = pygame.image.load('assets/cavalier_noir.png')
    ...
    def recup_all_pos(self,pos,plateau,check_echec = True):
        ...
        if -1 < x+1 < 8 and -1 < y+2 < 8:
            if not(plateau[(x+1,y+2)] in game.all_id_noirs):
                self.ajoute_pos(plateau.copy(),x,y,x+1,y+2,check_echec)
        if -1 < x-1 < 8 and -1 < y+2 < 8:
            if not(plateau[(x-1,y+2)] in game.all_id_noirs):
                self.ajoute_pos(plateau.copy(),x,y,x-1,y+2,check_echec)
        if -1 < x-2 < 8 and -1 < y+1 < 8:
            if not(plateau[(x-2,y+1)] in game.all_id_noirs):
