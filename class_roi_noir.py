...
class roi_noir(pygame.sprite.Sprite):
    ...
    def __init__(self,game,x,y,num):
        ...
        self.image = pygame.image.load('assets/roi_noir.png')
    ...
    def ajoute_pos(self,plateau,x,y,new_x,new_y,check_echec):
        ...
        del plateau[(x,y)]
        if check_echec:
            if not(game.is_echec(1,plateau)):
    ...
    def recup_all_pos(self,pos,plateau,check_echec = True):
        ...
        if -1 < x-1 < 8 and -1 < y-1 < 8:
            if not(plateau[(x-1,y-1)] in game.all_id_noirs):
                self.ajoute_pos(plateau.copy(),x,y,x-1,y-1,check_echec)

        if -1 < x < 8 and -1 < y-1 < 8:   
            if not(plateau[(x,y-1)] in game.all_id_noirs):
        ...
        if -1 < x+1 < 8 and -1 < y < 8:    
            if not(plateau[(x+1,y)] in game.all_id_noirs):
        ...
        if plateau.sens == 0:
            if y == 7 and x == 3 and plateau[(x-1,y)] == 0 and plateau[(x-2,y)] == 0 and plateau[(x-3,y)] == 25 and self.can_rock_noir:
                plateau_copy_1 = plateau.copy()
                plateau_copy_1[(x-1,y)] = plateau[(x,y)]
                del plateau_copy_1[(x-1,y)]
                if check_echec:
                    if not(game.is_echec(1,plateau_copy_1)):
                        plateau_copy_2 = plateau.copy()
                        plateau_copy_2[(x-2,y)] = plateau[(x,y)]
                        del plateau_copy_2[(x-2,y)]
                        if not(game.is_echec(1,plateau_copy_2)):
                            self.all_pos.append((x-2,y))
                else:
                    self.all_pos.append((x-2,y))
            if y == 7 and x == 3 and plateau[(x+1,y)] == 0 and plateau[(x+2,y)] == 0 and plateau[(x+3,y)] == 0 and plateau[(x+4,y)] == 32 and self.can_rock_noir:
                plateau_copy_1 = plateau.copy()
                plateau_copy_1[(x+1,y)] = plateau[(x,y)]
                del plateau_copy_1[(x+1,y)]
                if check_echec:
                    if not(game.is_echec(1,plateau_copy_1)):
                        ...
                        if not(game.is_echec(1,plateau_copy_2)):
                            ...
                            if not(game.is_echec(1,plateau_copy_3)):
                                self.all_pos.append((x+2,y))
                else:
                    self.all_pos.append((x+2,y))
        else:
            if y == 0 and x == 4 and plateau[(x+1,y)] == 0 and plateau[(x+2,y)] == 0 and plateau[(x+3,y)] == 25 and self.can_rock_noir:
                plateau_copy_1 = plateau.copy()
                plateau_copy_1[(x+1,y)] = plateau[(x,y)]
                del plateau_copy_1[(x+1,y)]
                if check_echec:
                    ...
                    self.all_pos.append((x+2,y))
            if y == 0 and x == 4 and plateau[(x-1,y)] == 0 and plateau[(x-2,y)] == 0 and plateau[(x-3,y)] == 0 and plateau[(x-4,y)] == 32 and self.can_rock_noir:
                plateau_copy_1 = plateau.copy()
                plateau_copy_1[(x-1,y)] = plateau[(x,y)]
                del plateau_copy_1[(x-1,y)]
                if check_echec:
    ...
    def remove(self):
        game = self.get_game()
        game.all_noirs_alive.remove(self)
